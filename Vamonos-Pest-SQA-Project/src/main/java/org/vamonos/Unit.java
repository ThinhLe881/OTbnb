...
public class Unit {
    ...
    // -> String
    // Return unit information as a string
    public String getUnitAsString() {
        return this.id + " " +
                Utils.pad(this.host, 16) +
                Utils.pad(this.city, 26) +
                Utils.pad(String.valueOf(this.bedrooms), 2) +
                Utils.padZeroes(Utils.parsePrice(this.price), 6) + " " +
                (this.rented ? "T " : "F ") +
                (this.rented ? Utils.padZeroes(String.valueOf(this.daysRemaining), 2) : "  ");
    }

    // -> null
    // Update number of days remaining at the end of the day
    public void updateDaysRemaining() {
        if (this.rented) {
            this.rented = !(--this.daysRemaining == 0);            
        }
    }
}
