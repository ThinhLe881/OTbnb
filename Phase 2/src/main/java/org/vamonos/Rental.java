...
public class Rental {
    ...
    public Transaction book(ArrayList<Unit> units, ArrayList<Unit> sessionUnits, User user) {
        String username = user.getUsername();

        Unit unit = this.queryUnit(units, sessionUnits, username);
        this.unit = unit;

        int numNights = this.queryNumNights();
        this.numNights = numNights;

        String dailyPrice = this.printDetails();

        boolean confirm = this.confirmBooking();
    ...
    private int queryNumNights() {
        ...
        String numNights = Utils.getInput(conditions, "Please enter the number of nights...");
    ...
    private Unit queryUnit(ArrayList<Unit> units, ArrayList<Unit> sessionUnits, String username) {
        Unit unit;
        String idToRent;

        LinkedHashMap<String, String> condition = new LinkedHashMap<>();
        condition.put(VALID_ID_FORMAT, "Sorry, that rental ID doesn't exist!");

        while(true) {
            idToRent = Utils.getInput(condition, "Please enter the rental ID...");
            if(null != Unit.getUnit(sessionUnits, idToRent)) {
                // If it is a new listing
                System.out.println("Sorry, that rental isn't available for rental yet!");
            } else {
                unit = Unit.getUnit(units, idToRent);
                if (null == unit) {
                    // Unit isn't found
                    System.out.println("Sorry, that rental ID doesn't exist!");
    ...
    private String printDetails() {
        ...
        System.out.printf("The rent per night is %s.%n", dailyPrice);
        System.out.printf("The total cost for %d day%s is %s%n", this.numNights, suffix, totalPrice);
    ...
    private boolean confirmBooking() {
        ...
        String confirmation = Utils.getInput(condition, "Please enter 'yes' to confirm your rental or 'no' to cancel.");
